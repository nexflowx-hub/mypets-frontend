...
/**
 * Section 1 — cinematic documentary hero (mockup: dark left overlay,
 * subject right, white headline, coral CTA, community proof, handwritten note)
 * with the FacePets panel docked at the right on xl screens.
 */
export function HeroSection() {
  ...
  const scrollToStories = () =>
    document.querySelector("#historias")?.scrollIntoView({ behavior: "smooth" });
  ...
  return (
    <section id="top" className="xl:grid xl:grid-cols-[minmax(0,1fr)_330px] 2xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <div className="relative isolate min-h-[92svh] overflow-hidden bg-petrol sm:min-h-[86svh] xl:min-h-[calc(100svh-68px)] xl:max-h-[860px]">
        ...
        />
        {/* cinematic overlays: strong left gradient + bottom fade */}
        <div
          aria-hidden
          className="absolute inset-0 bg-gradient-to-r from-[#0b161d]/92 via-[#0b161d]/62 to-black/10"
        />
            ...
            <div className="mt-9 flex items-center gap-3.5">
              <div className="flex -space-x-2.5" aria-hidden>
                {AVATARS.map((a) => (
                  <span
                    key={a.src}
                    className="relative block h-9 w-9 overflow-hidden rounded-full ring-2 ring-white/70"
                  >
                    <Image src={a.src} alt="" fill sizes="36px" className="object-cover" />
                  </span>
                ))}
              </div>
              <p className="max-w-[240px] text-[12.5px] font-medium leading-snug text-white/85">
                {dict.hero.community}
              </p>
            </div>
          </div>
      ...
      {/* ── FacePets docked panel (xl+) ── */}
      <FacePetsPanel className="hidden xl:flex xl:max-h-[calc(100svh-68px)]" />
    </section>
  );
}

/** Standalone FacePets block for < xl screens */
