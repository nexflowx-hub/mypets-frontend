...
export function SiteHeader() {
  ...
  return (
    <header
      className={cn(
        "fixed inset-x-0 top-0 z-50 bg-white/95 backdrop-blur-md transition-shadow",
        scrolled ? "shadow-[0_1px_0_0_var(--border),0_8px_24px_-16px_rgba(16,32,42,0.35)]" : "border-b border-transparent"
      )}
    >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              className="hidden h-10 items-center gap-1.5 rounded-full border border-border bg-white px-3 text-[13px] font-semibold text-ink/85 transition-colors hover:border-coral/50 hover:text-coral focus-visible:outline-2 focus-visible:outline-coral sm:flex"
            >
            ...
            <DropdownMenuContent align="end" className="min-w-44 rounded-xl border-border p-1.5">
              {LOCALES.map((l: Locale) => (
                <DropdownMenuItem
                  ...
                  onClick={() => setLocale(l)}
                  className={cn(
                    "cursor-pointer gap-2.5 rounded-lg px-3 py-2.5 text-[13.5px] font-semibold",
                    l === locale && "bg-accent text-coral"
                  )}
                >
                  <span aria-hidden className="text-base">{LOCALE_META[l].flag}</span>
                  {l === "en" ? "English" : l === "pt-BR" ? "Português (BR)" : "Português (PT)"}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            <SheetTrigger
              ...
              className="rounded-full p-2.5 text-ink transition-colors hover:bg-accent focus-visible:outline-2 focus-visible:outline-coral xl:hidden"
            >
              ...
              <nav className="flex flex-col gap-1 px-3 py-4" aria-label={dict.nav.menu}>
                {navItems.map((item) => (
                  <button
                    key={item.label}
                    onClick={() => go(item.href)}
                    className="rounded-lg px-3 py-3 text-left text-[15px] font-semibold text-ink transition-colors hover:bg-accent hover:text-coral"
                  >
                    {item.label}
                  </button>
                ))}
