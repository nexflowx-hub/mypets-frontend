...
import { CommunityCauseDisclosure, type SubmittedSocialLink } from "@/components/causes/community-cause-disclosure";
import { ArrowRight, BadgeCheck, HeartHandshake, MapPin, PawPrint, ShieldCheck } from "lucide-react";
...
export default async function CausePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const progress = cause.targetAmountCents
    ...
    : 0;
  const community = cause.beneficiaryKind === "COMMUNITY";
  const verified = cause.verificationStatus === "VERIFIED" || cause.verificationStatus === "PLATFORM";
      ...
      <main className="min-h-screen bg-cream pt-[68px]">
          ...
          <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_0.9fr] lg:px-8 lg:py-16">
            <div className="flex flex-col justify-center">
              <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-white/65">
                <span className="inline-flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" />{cause.city ? `${cause.city}, ` : cause.region ? `${cause.region}, ` : ""}{cause.country}</span>
                ...
                {verified && <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-400/15 px-2.5 py-1 text-emerald-200"><BadgeCheck className="h-3.5 w-3.5" />MyPets Verificado</span>}
                {community && <span className="rounded-full bg-amber-300/15 px-2.5 py-1 text-amber-200">Enviada pela comunidade</span>}
              </div>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                {financialCause && paymentCurrency && <CauseCheckout causeId={cause.id} causeTitle={cause.title} currency={paymentCurrency} enabled={checkoutEnabled} />}
        ...
        <div className="mx-auto grid max-w-7xl gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:px-8 lg:py-14">
          <div className="space-y-7">
            {community && <CommunityCauseDisclosure socialLinks={cause.submittedSocialLinks ?? []} publicWhatsapp={cause.publicWhatsapp ?? null} />}
          ...
          <aside className="space-y-5">
            <div className="rounded-3xl border border-border bg-white p-6">
              ...
              {financialCause && cause.targetAmountCents && cause.currency && (
              ...
              {community && <div className="mt-4 rounded-2xl bg-amber-50 p-4 text-xs leading-5 text-amber-950"><strong>Apoios financeiros ainda não disponíveis.</strong> Esta causa precisa concluir a verificação antes de receber pagamentos através do MyPets.</div>}
            </div>
